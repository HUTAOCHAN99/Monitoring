...
class SensorRemoteDataSource {
  ...
  void _fetchLatestDataWithPolling(StreamController<Map<String, dynamic>> controller) async {
    try {
        ...
        if (sensorData.isNotEmpty) {
          ...
          if (deviceResponse.statusCode == 200) {
            ...
            if (deviceData.isNotEmpty) {
              ...
              final control = AutomaticControl.kontrolOtomatis(sensor.suhu, sensor.kelembapan);
          ...
          _log('   - Sensor: ${sensor.suhu}°C, ${sensor.kelembapan}% at ${sensor.timestamp}');
          _log('   - Device: Lampu=${device.statusLampu}, Jumlah Lampu=${device.jumlahLampu}, Humidifier=${device.statusHumidifier} at ${device.timestamp}');
  ...
  Future<void> _triggerAutomaticControlOnNewData(SensorData sensorData) async {
    try {
      ...
      final control = AutomaticControl.kontrolOtomatis(sensorData.suhu, sensorData.kelembapan);
  ...
  Future<List<Map<String, dynamic>>> fetchHistoricalData({int limit = 20}) async {
    try {
      _log('📚 Fetching historical data (limit: $limit)...');
      
      final sensorResponse = await http.get(
        Uri.parse('${AppConstants.supabaseUrl}/rest/v1/sensor_data?order=timestamp.desc&limit=$limit'),
        headers: _getHeaders(),
      );

      final deviceResponse = await http.get(
        Uri.parse('${AppConstants.supabaseUrl}/rest/v1/device_status?order=timestamp.desc&limit=$limit'),
        headers: _getHeaders(),
      );
      ...
      if (sensorResponse.statusCode == 200 && deviceResponse.statusCode == 200) {
        ...
        for (int i = 0; i < sensorData.length; i++) {
          ...
          historicalData.add({
            'sensor': sensor,
            'device': closestDevice ?? DeviceStatusModel(
              id: 0,
              statusLampu: false,
              jumlahLampu: 0,
  ...
  Future<bool> sendAutomaticControl(double suhu, double kelembapan) async {
    try {
      ...
      _log('   - Lampu: $lampuValue (${AutomaticControl.getStatusDescription(lampuValue, mistValue)['lampu']})');
      _log('   - Mist: $mistValue (${AutomaticControl.getStatusDescription(lampuValue, mistValue)['mist']})');
